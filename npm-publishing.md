...
git add README.md package.json src tests LICENSE npm-publishing.md
...
- If your npm account enforces an OTP on top of a publish token, rerun with `npm publish --otp=<code>` and the same `--//registry.npmjs.org/:_authToken="$NPM_TOKEN"` flag.
